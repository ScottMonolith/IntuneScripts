...
function Install-ADTDeployment
{
    ...
    $AF = Get-ADTApplication -Name 'PI AF Client'
    $PIDL64 = Get-ADTApplication -Name 'PI DataLink * x64'
    $PIDL32 = Get-ADTApplication -Name 'PI DataLink * x86'
    if ($AF) {
        $AF | Uninstall-ADTApplication
    }
    if ($PIDL64) {
        $PIDL64 | Uninstall-ADTApplication
    }
    if ($PIDL32) {
        $PIDL32 | Uninstall-ADTApplication
    }
    if (!($AF) -and !($PIDL64) -and !($PIDL32)) {
        Write-ADTLogEntry -Message "No other versions found, continuing with installation..." -Source 'Pre-Install'
    }
    ...
    Start-ADTProcess -FilePath "Setup.exe" -Argumentlist "-f silent.ini"
    ...
    Show-ADTInstallationPrompt -Title "$($adtSession.AppName) $($adtSession.AppVersion)" -Message "Installation Complete!" -ButtonRightText 'OK' -Icon Information
}
...
function Uninstall-ADTDeployment
{
    ...
    ## Show Progress Message (with the default message).
    Show-ADTInstallationProgress -WindowTitle "$($adtSession.AppName) $($adtSession.AppVersion)" -WindowSubtitle "Monolith Materials - App Uninstallation" -StatusMessage "Uninstallation in Progress..." -StatusMessageDetail "The uninstallation process will take a while to complete, please be patient." -NotTopMost
    ...
    Show-ADTInstallationPrompt -Title "$($adtSession.AppName) $($adtSession.AppVersion)" -Message "Uninstallation Complete!" -ButtonRightText 'OK' -Icon Information
}
